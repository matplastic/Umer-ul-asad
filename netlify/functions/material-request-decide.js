...
exports.handler = async (event) => {
  ...
  try {
    if (!id || !token || !action) return html('Invalid Link', 'Missing request details.', false, 400);
    ...
    if (idx === -1) return html('Not Found', 'This material request no longer exists.', false, 404);
    const item = arr[idx];
    if (item.approvalToken !== token) return html('Invalid Link', 'This approval link is not valid.', false, 403);
    if (item.status !== 'PENDING') return html('Already Decided', `This request was already marked as ${item.status}.`, item.status === 'APPROVED');
