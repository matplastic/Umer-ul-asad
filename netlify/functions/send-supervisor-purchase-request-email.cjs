// Netlify Function — sends the "new Factory Supervisor purchase request"
// email to the manager. Runs server-side, so the Resend API key never
// reaches the browser.
//
// Sends ONE email per batch (the supervisor's whole cart — 1 to many item
// lines), with a line-items table, mirroring send-hr-purchase-request-email.cjs.
//
// PER-ITEM APPROVE/REJECT: "Review & Decide Items" opens a page (see
// netlify/functions/supervisor-purchase-request-decide.cjs) that lists every
// item line with its own Approve/Reject choice, so out of e.g. 5 items the
...
//   RESEND_API_KEY, SUPERVISOR_EMAIL_FROM (optional), SUPERVISOR_MANAGER_EMAIL
//   (falls back to HR_MANAGER_EMAIL, then STORE_MANAGER_EMAIL, if not set),
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const managerEmails = (process.env.SUPERVISOR_MANAGER_EMAIL || process.env.HR_MANAGER_EMAIL || process.env.STORE_MANAGER_EMAIL || 'hussein.khalil@matglobal.tech')
      .split(',').map((e) => e.trim()).filter(Boolean);

    if (!apiKey || managerEmails.length === 0) {
      console.warn('[send-supervisor-purchase-request-email] RESEND_API_KEY or SUPERVISOR_MANAGER_EMAIL not set — skipping.');
    ...
    const reviewUrl = `${siteUrl}/.netlify/functions/supervisor-purchase-request-decide?${idOrBatchQuery}&token=${payload.approvalToken}`;
    ...
    const subject = items.length > 1
      ? `Factory Purchase Request: ${items.length} items`
      : `Factory Purchase Request: ${items[0].itemName || 'Item'} (${items[0].category || 'Tools'})`;
    ...
    const html = `
      <div style="font-family:Arial,sans-serif; max-width:560px; margin:0 auto; color:#1e293b;">
        <div style="background:#0f172a; padding:20px 24px; border-radius:10px 10px 0 0; display:flex; align-items:center; gap:12px;">
          ...
          <h2 style="color:#fff; margin:0; font-size:15px; text-transform:uppercase; display:inline-block; vertical-align:middle;">MAT Plastic Industries LLC — Factory Supervisor</h2>
        </div>
        <div style="border:1px solid #e2e8f0; border-top:none; padding:24px; border-radius:0 0 10px 10px;">
          ...
          <table style="width:100%; border-collapse:collapse; font-size:14px; margin-bottom:16px;">
            ...
            ${payload.sectionName ? `<tr><td style="padding:6px 0; color:#64748b;">Section</td><td style="padding:6px 0;">${payload.sectionName}</td></tr>` : ''}
    ...
    if (!res.ok) {
      ...
      console.error('[send-supervisor-purchase-request-email] Resend error:', res.status, detail);
      return { statusCode: 502, body: JSON.stringify({ error: 'Failed to send email', detail }) };
    }

    return { statusCode: 200, body: JSON.stringify({ success: true }) };
  } catch (err) {
    console.error('[send-supervisor-purchase-request-email] Error:', err);
